...
#backup world
Radiobutton(master, text = "Start Server", variable=var, value = 6).grid(row=6, sticky=W)
Button(master, text = "GO!", command=quit_loop).grid(row=7, sticky=W)
...
if selection == 5:
    ...
    file_path = filedialog.askopenfilename()
    subprocess.call(["rm", "-rf", "world", ";", "cp", file_path, "server"])
...
if MsgBox == 'yes':
    ...
    subprocess.call(["open", cwd, "-n", "-a", "/System/Applications/Utilities/Terminal.app"])# ";", "./ngrok", "tcp", "25565"])
if MsgBox == 'no':
    os.system("cd server ; open server.properties /System/Applications/TextEdit.app") 
